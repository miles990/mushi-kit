...
function buildDailyPayloadPath(basePath, date) {
...
function parseSseTokenUsageFromBody(text) {
...
function cleanupOldPayloadLogs(basePath, now) {
    const dir = dirname(basePath);
    if (!existsSync(dir))
        return;
    const file = basename(basePath);
    ...
    const cutoff = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate() - PAYLOAD_RETENTION_DAYS));
...
function logPayload(basePath, entry) {
    try {
        const now = new Date();
...
function logRequest(path, entry) {
    const dir = dirname(path);
    if (!existsSync(dir))
        mkdirSync(dir, { recursive: true });
    appendFileSync(path, JSON.stringify(entry) + '\n', 'utf-8');
...
async function forwardRequest(target, method, path, headers, body) {
...
async function forwardStreaming(target, method, path, headers, body, res, logPath, payloadLogPath, hash) {
    const url = `${target}${path}`;
...
export function startProxy(config) {
    const { port, target, apiKey, shadow = true, cachePath = './myelin-cache.json', logPath = './myelin-proxy.jsonl', payloadLogPath = './payload.jsonl', minHits = 3, } = config;
    if (payloadLogPath) {
    ...
    const server = createServer(async (req, res) => {
        ...
        try {
            ...
            const source = shadow ? 'shadow' : 'forwarded';
            logRequest(logPath, {
                ts: new Date().toISOString(),
                method: 'POST', path: req.url ?? '/', hash,
                source,
