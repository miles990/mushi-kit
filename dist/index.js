 ...
 * ```
 */
import { findMatchingRule, loadRules, saveRules, generateRuleId } from "./rules.js";
import { logDecision, readDecisionLog, logCrystallization } from "./telemetry.js";
import { findCandidates, candidateToRule } from "./crystallizer.js";
import { extractTemplates } from "./templates.js";
import { extractMethodology, formatMethodology } from "./methodology.js";
import { scoreAlignment, adjustedThreshold, buildGuidance, optimizeRules as optimizeRulesFn, detectEvolution, } from "./feedback-loop.js";
// Re-export utilities for advanced usage
export { matchRule, findMatchingRule, loadRules, saveRules } from "./rules.js";
export { logDecision, readDecisionLog, getLlmDecisions, logCrystallization } from "./telemetry.js";
export { findCandidates, candidateToRule } from "./crystallizer.js";
export { extractTemplates, mergeTemplateToRuleFromRules } from "./templates.js";
export { extractMethodology, formatMethodology } from "./methodology.js";
export { scoreAlignment, adjustedThreshold, buildGuidance, optimizeRules, detectEvolution } from "./feedback-loop.js";
export { startProxy } from "./proxy.js";
export { createFleet } from "./fleet.js";
export { createStack } from "./stack.js";
export { getOrCreate, getInstance, removeInstance, listInstances, clearInstances } from "./singleton.js";
 ...
 * ```
 */
export function createMyelin(config) {
    ...
    function crystallize(candidate) {
    ...
    function observe(event, metadata) {
        observeCount++;
        if (autoLog) {
            const reason = metadata ? JSON.stringify(metadata) : 'observation';
            logDecision(logPath, event, 'observe', reason, 'observe', 0);
        }
    }
    async function triageSafe(event) {
        try {
    ...
    function distill() {
        ...
        const standardCandidates = findCandidates(logs, {
            minOccurrences,
            minConsistency,
            methods: crystallizeMethods,
        });
        ...
        for (const candidate of allCandidates) {
            const alreadyCovered = rules.some(r => r.action === candidate.suggestedAction &&
    ...
    function crystallizeEpisodes(opts) {
        ...
        for (const [pattern, data] of patterns) {
            if (data.episodes.length < minEps)
                continue;
    ...
    function maybeDistill(opts) {
        ...
        if (newDecisions < minNew && elapsed < minInterval)
            return null;
    ...
    function toSmallModelPrompt() {
        const templates = extractTemplates(rules);
        const methodology = extractMethodology(templates, rules);
        if (methodology.principles.length === 0 && rules.length === 0)
            return '';
        const lines = [];
        if (methodology.principles.length > 0) {
            lines.push('RULES:');
