...
function ruleFingerprint(rule) {
    const parts = [`action:${rule.action}`];
    if (rule.match.type) {
 ...
 */
function extractInvariants(rules) {
    const invariants = { stableContext: {} };
    ...
    if (rules.every(r => r.match.context)) {
        ...
        for (const key of allKeys) {
            ...
            if (valuesForKey.length !== rules.length)
                continue;
            // Check if all have the same condition type
            const types = new Set(valuesForKey.map(v => conditionType(v)));
            if (types.size === 1) {
                invariants.stableContext[key] = [...types][0];
            }
        }
    }
    return invariants;
 ...
 */
function findVariables(rules, stableKeys) {
    const allKeys = new Set();
    for (const rule of rules) {
        if (rule.match.context) {
            for (const key of Object.keys(rule.match.context)) {
 ...
 */
function generateTemplateName(action, invariants, ruleCount) {
    const parts = [];
 ...
 */
export function extractTemplates(rules, opts = {}) {
 ...
 */
export function mergeTemplateToRule(template) {
    const match = {};
 ...
 */
export function mergeTemplateToRuleFromRules(template, rules) {
