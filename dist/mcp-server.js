...
import { logDecision, readDecisionLog } from "./telemetry.js";
...
export function createMcpServer(opts = {}) {
    ...
    const server = new McpServer({ name: 'myelin', version: '0.1.0' }, { capabilities: { tools: {} } });
    // --- myelin_check ---
    server.tool('myelin_check', 'Check if a rule matches an event (instant, 0 tokens)', {
        event: z.object({
            type: z.string(),
        ...
        const matched = findMatchingRule(triageEvent, rules);
        if (matched) {
        ...
        return {
            content: [{ type: 'text', text: JSON.stringify({ hit: false }) }],
        };
    });
    // --- myelin_record ---
    server.tool('myelin_record', 'Record an LLM decision for future crystallization', {
        event: z.object({
            type: z.string(),
            source: z.string().optional(),
            context: z.record(z.string(), z.unknown()).optional(),
    ...
    // --- myelin_crystallize ---
    server.tool('myelin_crystallize', 'Find stable patterns and crystallize them into rules', {
    ...
    // --- myelin_stats ---
    server.tool('myelin_stats', 'Get current myelin statistics', {}, async () => {
        const rules = loadRules(rulesPath);
        const logs = readDecisionLog(logPath);
    ...
    // --- myelin_rules ---
    server.tool('myelin_rules', 'List/manage crystallized rules', {
        ...
        if (action === 'add') {
            if (!rule) {
                return {
                    content: [{ type: 'text', text: JSON.stringify({ error: 'rule is required for add action' }) }],
                    isError: true,
                };
            }
            const newRule = {
            ...
            saveRules(rulesPath, rules);
            return {
                content: [{ type: 'text', text: JSON.stringify({ added: newRule }) }],
            };
        }
        if (action === 'remove') {
            if (!id) {
                return {
                    content: [{ type: 'text', text: JSON.stringify({ error: 'id is required for remove action' }) }],
                    isError: true,
                };
            }
            ...
            if (idx === -1) {
                return {
                    content: [{ type: 'text', text: JSON.stringify({ error: `rule ${id} not found` }) }],
                    isError: true,
                };
            }
        ...
        return {
            content: [{ type: 'text', text: JSON.stringify({ error: 'unknown action' }) }],
            isError: true,
        };
    });
    return server;
...
const isDirectRun = process.argv[1] && (process.argv[1].endsWith('mcp-server.ts') ||
