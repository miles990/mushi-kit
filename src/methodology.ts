...
import type { Template, Methodology, Dimension, Principle, MatrixCell, Rule } from './types.ts';
 ...
 */
export function extractMethodology<A extends string>(
...
function emptyMethodology(): Methodology {
  return {
    dimensions: [],
    principles: [],
    matrix: [],
 ...
 */
function extractDimensions<A extends string>(templates: Template<A>[]): Dimension[] {
  // Collect all context keys and their types across templates
  const keyInfo = new Map<string, {
    types: Set<string>;
...
function deriveLevels(types: Set<string>): string[] {
...
function deriveDescription(key: string, types: Set<string>): string {
 ...
 */
function extractPrinciples<A extends string>(
  templates: Template<A>[],
  dimensions: Dimension[],
): Principle[] {
  // Group templates by action
  const byAction = new Map<A, Template<A>[]>();
  for (const template of templates) {
...
function findCommonDimensions<A extends string>(
  templates: Template<A>[],
...
function getCommonConditionType<A extends string>(
  templates: Template<A>[],
  ...
  if (types.size === 0) return 'present';
...
function computeConfidence<A extends string>(
  actionTemplates: Template<A>[],
  allTemplates: Template<A>[],
): number {
 ...
 */
function buildMatrix<A extends string>(
  templates: Template<A>[],
  rules: Rule<A>[],
  dimensions: Dimension[],
): MatrixCell[] {
 ...
 */
export function formatMethodology(methodology: Methodology): string {
  const lines: string[] = [];
