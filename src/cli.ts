...
import { readDecisionLog } from './telemetry.ts';
...
function cmdInit(): void {
  ...
  if (existsSync(cursorDir)) {
    try {
      ...
      (existing as Record<string, unknown>).mcpServers = (existing as Record<string, Record<string, unknown>>).mcpServers ?? {};
      (existing as Record<string, Record<string, unknown>>).mcpServers.myelin = mcpConfig;
